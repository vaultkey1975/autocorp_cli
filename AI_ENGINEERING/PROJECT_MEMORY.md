...
**A second, independent instance of the same lesson: an investigation report's claim can be wrong even when it was carefully evidence-based and written minutes earlier — always re-verify empirically before acting on it, including your own prior session's findings.** The Reliability Engine investigation (2026-07-29) claimed a missing `ruff`/`flake8`/`mypy` binary would be "treated as a blocking static-gate issue" for every edit. Re-verifying by actually monkeypatching tool detection to simulate the missing-tool case and calling `StaticGate.run_delta` (the call site checked that day, via `ReliabilityTestLoop` in `test_loop.py`) showed it does NOT block there — the missing-tool marker appears identically in both the before/after snapshots `run_delta` diffs, so it's never flagged as "new." The claim was called "true of `StaticGate.run()`, an unused alternate method" and the finding was documented as a correction rather than a fix. **That conclusion was itself incomplete** — see the next entry.

**A third instance, one day later, correcting the second: "unused" was not verified, it was assumed from checking only the one call site already under discussion.** On 2026-07-30, a production-readiness review asked to re-check every finding once more before deciding whether to integrate. Actually grepping every call site of `StaticGate.run(` — not just the one already known from the day before — found `SelfConsistencyRunner.choose()` (in `self_consistency.py`, a different file from the one checked the previous day) calls the "unused alternate method" directly. It is not unused. In an environment missing `ruff`/`mypy`, `choose()` would genuinely reject every self-consistency candidate for exactly the high-blast-radius/core-touching changes that mechanism exists to protect — a real, live bug, missed by both the original investigation and the first re-verification, because both checked the same one call site rather than enumerating all of them. Fixed by making `choose()` match its sibling `choose_edit()`'s already-correct `collect_issues()` + `run_delta()` pattern, with a regression test confirmed to fail against the pre-fix code. **The lesson underneath both entries: re-verifying a claim by checking the one code path that prompted it is not the same as re-verifying the claim itself — "is X true here" and "is X true anywhere this matters" are different questions, and conflating them is exactly how a real bug survived two rounds of deliberate, careful re-checking.** When a finding involves "does A do X," enumerate every caller of A before concluding X does or doesn't matter — don't stop at the first one found.

**Treat "an investigation already covered this" as a reason to re-verify quickly, not a reason to skip verification** — see also the Phase 1G audit entry above, a separate concrete instance of the identical pattern, now joined by two more from the same day and the next.
