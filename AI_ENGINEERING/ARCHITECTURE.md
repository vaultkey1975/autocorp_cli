...
Subcommands committed before the discovery change (`git show ff31c1a:autocorp.py`):
...
Discovery-backed Chat routes (`show repository profile`, `show
...
- **Universal Repository Discovery:** `discovery.py` profiles unknown
  repositories from evidence: languages, frameworks, package managers,
  build/test/lint/format/type tools, database technology, containers,
...
**Discovery addition:** `repository_profiles` stores AutoCorp metadata
generated by `brains/discovery.py`: repository path/name and the complete
JSON discovery profile. This table is written only in AutoCorp's own
SQLite database under `data/`; discovery never writes metadata into the
target repository.
...
1. `scanner.run_scan(repo_path)` for git status and raw file/code-health
   markers.
2. `discovery.discover_repository(repo_path)` automatically when no stored
   profile exists for the target path in AutoCorp metadata.
3. `analyzer.run_analysis(repo_path)` for architecture, entry point,
   dependency, and test-framework evidence.
4. `project_planner.run_project_plan(repo_path)` for prioritized
   repository actions and blockers.
5. `live_readiness.run_live_readiness(repo_path)` for static production
   readiness checks. If this raises, the manager reports the readiness
   scanner error as a production-readiness deduction; it does not fabricate
   a pass.
6. Read-only git commands (`git log`, `git status`) for recent-change
   evidence.
7. `AI_ENGINEERING/CURRENT_PHASE.md` from the target repository, if
   present, for current-phase evidence; otherwise it reports "Unable to
   determine from repository evidence."
8. Existing command surfaces for workflow-test, publish-test, repair, and
...
`brains/discovery.py` is the first-stage profile engine for repositories
that have never been seen by AutoCorp. It is read-only against the target
repository and stores only AutoCorp metadata through `memory/store.py`.

Discovery reuses existing evidence modules first:

1. `scanner.run_scan(repo_path)` for git status and Python/test/code-health
   counts.
2. `analyzer.run_analysis(repo_path)` for project-type, entry-point,
   dependency-file, and test-framework evidence.

It then inspects standard repository manifests/configs: `requirements.txt`,
`pyproject.toml`, `package.json`, `Cargo.toml`, `go.mod`, `pom.xml`,
`build.gradle`, `.csproj` / `.sln`, `CMakeLists.txt`, `Dockerfile`,
Compose files, GitHub Actions, GitLab CI, Azure Pipelines, `Makefile`,
`Taskfile`, README files, `docs/`, `tests/`, `src/`, `examples/`, and
`AI_ENGINEERING/`.

Technology-stack inference uses maintained-source evidence. Discovery
excludes generated/runtime `workspace/`, `data/`, virtualenvs, caches, and
build outputs so disposable generated repositories do not pollute the
profile of the target repository.

Every profile conclusion carries evidence and confidence. Missing evidence
