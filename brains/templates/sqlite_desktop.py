#!/usr/bin/env python3
"""
SQLite Desktop template  (AutoCorp CLI - brains.templates)  [SQLite Gen Phase 3]
===============================================================================
...
deterministic plan for a PySide6 desktop app with a SQLite persistence layer:
    ...
    +-- ui/
        ...
        +-- main_window.py   QMainWindow: master-detail UI wired to crud.*

Phase 3: the UI is a MASTER-DETAIL view. The master is a table of the primary
entity (customers) annotated with child counts (crud.get_<primary>_with_counts);
selecting a row reveals that row's children (notes, interactions) in detail lists,
with add/delete for each child. database.py, crud.py, main.py, requirements.txt and
ui/__init__.py are emitted as exact `content` (written verbatim by the Builder);
only ui/main_window.py is generated by the engine, from a purpose that names the
exact crud functions to call.
...
def _main_window_purpose(schema, app_desc):
    """The Phase 3 master-detail UI purpose, derived from the schema. The master is
    the primary table (with child counts); selecting a row reveals its children."""
    primary = schema[0]
    table = primary.name
    ...
    purpose = (
        ...
        "title with setWindowTitle(); set the size with resize(1000, 650); create "
        "a central QWidget and pass it to self.setCentralWidget(central); create a "
        "QVBoxLayout and apply it to that central widget; initialise "
        "self.selected_id = None; and call crud.init_db() FIRST so the database "
        "and all tables exist before any query.\n"
        ...
        f"crud.search_{table}(self.search_input.text()); and a 'Refresh' "
        f"QPushButton -> crud.get_{table}() / {master_source}.\n"
        + (detail + "\n" if detail else "")
        + "Use ONLY crud functions that exist (the exact names above). Store every "
        "widget a method refers to as an attribute on self so there are NO "
...
def build_plan(request: str) -> dict:
    ...
    return {
        ...
        "success_criteria": [
            ...
            "master table with a detail pane (children of the selected row) "
            "supporting add / edit / delete / search / refresh",
